...
import { ContextMenu, Grid, Resize, Sort, VirtualScroll, getUid } from '@syncfusion/ej2-grids';
...
import { ContextMenu as ContextMenu$1, Toolbar, TreeView } from '@syncfusion/ej2-navigations';
        ...
        if (isFileSystemData(parent)) {
            if (args.isCopy) {
                ...
                copiedFolders.forEach((itemData) => {
                    if (!isFileExists(folderSubItems, itemData.name)) {
                        copyFolderItems(parent, itemData, args.targetData, null);
                    }
                    else {
                        const names = itemData.name.split('.');
                        const name = names[0] + '(' + parent.existingFileCount + ').' + names[1];
    ...
    parent.trigger('beforeSend', eventArgs, (beforeSendArgs) => {
        if (!beforeSendArgs.cancel) {
            ...
            if (isFileSystemData(parent)) {
                ...
                const idValue = event === 'paste-end' && (parent.targetModule === 'largeiconsview' || parent.targetModule === 'detailsview')
                    ? getValue('data', data)[0].id : pathArray[pathArray.length - 1];
                const action = getValue('action', data);
                const isFileOperation = action === 'move' || action === 'rename' || action === 'copy' || action === 'delete' || action === 'search';
                if (action === 'read' || action === 'create') {
                    ...
                    const totalSize = isMultipleFiles ? itemData.reduce((accumulator, currentObject) => accumulator + (currentObject.size || 0), 0) + ' B' : details.size + ' B';
                    const path = (parent.pathNames.includes(details.name) ? parent.pathNames : parent.pathNames.join('/') + '/' + details.name);
                    parent.responseData.details = Object.assign({ location: isMultipleFiles ? null : path, multipleFiles: isMultipleFiles, name: itemNames.join(', '), size: totalSize }, (isMultipleFiles ? {} : {
...
class NavigationPane {
    ...
    addChild(files, target, prevent) {
        ...
        if (directories.length > 0 &&
            (directories[0].filterPath == null || isNullOrUndefined(this.getTreeData(target)[0])
                && this.getTreeData(target)[0].filterPath == null
                || directories[0].filterPath !== this.getTreeData(target)[0].filterPath)) {
            let length = 0;
            // eslint-disable-next-line
